...
{
	//aroth:  url's may be up to MAXREQUEST in size; do not use MAXBUF here unless we're sure everything will fit
	int     i, j, bufSize;
	char* code_msg, rep, cont, safe_url;

	bufSize = (strlen(url) < MAXBUF - 1024) ? MAXBUF : MAXREQUEST;		//aroth:  only allocate 'MAXREQUEST' if we really need to

	rep = malloc(bufSize);
    cont = malloc(bufSize);
    safe_url = malloc(bufSize);
	 ...
	 */
    //aroth:  add curly braces, use 'bufSize' instead of MAXBUF
	memset(safe_url, 0, bufSize);
	for(i = j = 0; i < bufSize && j < bufSize && url[i]; i++) {
...
{
	//aroth:  unsure what this will be called with; use MAXREQUEST to be safe
    //char        buf[MAXBUF];
	char* buf = malloc(cont > MAXREQUEST ? MAXREQUEST: cont);
    int         res;

    while(cont > L0) {
        if((res = BIO_read(cl, buf, cont > MAXREQUEST ? MAXREQUEST: cont)) < 0) {
            free(buf);
        	return -1;
        }
        else if(res == 0) {
...
copy_chunks(BIO *const cl, BIO *const be, LONG *res_bytes, const int no_write, const LONG max_size)
{
	//aroth:  unsure what this will be called with; use MAXREQUEST to be safe
    //char        buf[MAXBUF];
	char* buf = malloc(MAXREQUEST);
    LONG        cont, tot_size;
    regmatch_t  matches[2];
    int         res;

    for(tot_size = 0L;;) {
        if((res = get_line(cl, buf, MAXREQUEST)) < 0) {
        	free(buf);
...
get_headers(BIO *const in, BIO *const cl, const LISTENER *lstn)
{
    char    **headers;
    //char buf[MAXBUF];
    char* buf = malloc(MAXREQUEST);  //aroth:  use the configured max-request size here to ensure that we don't reject valid URL's as being too long
    int     res, n, has_eol;

    /* HTTP/1.1 allows leading CRLF */
    memset(buf, 0, MAXREQUEST);
    while((res = BIO_gets(in, buf, MAXREQUEST - 1)) > 0) {
    ...
    //XXX:  aroth: the first "header" appears to be used to store the HTTP verb and path, so we should use MAXREQUEST here to ensure that arbitrarily large paths are honored
    if((headers[0] = (char *)malloc(MAXREQUEST)) == NULL) {
    ...
    memset(headers[0], 0, MAXREQUEST);
    strncpy(headers[0], buf, MAXREQUEST - 1);
    ...
    for(n = 1; n < MAXHEADERS; n++) {
        ...
        //XXX:  aroth: should be okay to use MAXBUF below here; we want to allow a large REQUEST, but not allow massive values for each individual HEADER
...
do_http(thr_arg *arg)
{
    ...
    //aroth:  these things are unrelated to request buffering
    char				**headers, *mh, req_time[LOG_TIME_SIZE], s_res_bytes[LOG_BYTES_SIZE], headers_ok[MAXHEADERS];

    //aroth:  these things should be able to use MAXBUF
    char				v_host[MAXBUF], referer[MAXBUF], u_agent[MAXBUF], u_name[MAXBUF],
    					caddr[MAXBUF];

    //aroth:  these things should use MAXREQUEST
    //FIXME:  just rework the entire thing for pure dynamic allocation based upon the request size?
    int maxLen = MAXREQUEST;
    int checkBytes = 0;
    int bytesRead = 0;
    int bytesToCopy = 0;
    int parseBody = -1;
    ...
    BIO_set_buffer_size(cl, workingBufferSize);

    cl = BIO_push(bb, cl);

    //aroth:  check to see if we need to use a larger buffer size
    memset(buf, 0, MAXBUF);
    memset(safeBuf, 0, MAXREQUEST);
    while((bytesRead = BIO_gets(cl, buf, MAXBUF - 1)) > 0) {
		...
		if (parseBody < 0) {
			//aroth:  we only check HTTP verbs defined in RFC 2616 here; the consequences of making the wrong choice here are that the request body will be dropped (if parseBody is erroneously set to 0) or the request will time-out (if parseBody is erroneously set to 1 and 'Content-Length: 0' is not sent)
			if (buf[0] == 'P' || buf[0] == 'T' || buf[0] == 'O') {
				//POST, PUT, TRACE, and OPTIONS; for these we assume that we need to parse(and preserve) the request body, unless we find a 'Content-Length: 0' header
				parseBody = 1;
			}
			else {
				parseBody = 0;
		...
		if (strstr(buf, "Content-Length:") == buf || strstr(buf, "content-length:") == buf || strstr(buf, "Content-length:") == buf) {
			//aroth:  if the client indicates that there is no body content, disable parsing of the body content
			char* numericPart = buf + strlen("Content-Length:");
	...
	if (checkBytes > MAXBUF) {
		if (checkBytes > maxLen) {
			//aroth:  don't even bother proceeding; the request is too large
			logmsg(LOG_WARNING, "(%lx) e414 uri too long; length=%d, MAXREQUEST=%d", pthread_self(), checkBytes, MAXREQUEST);
			err_reply(cl, h414, lstn->err414);
			free(buf);
			free(safeBuf);
			return;
		}
		workingBufferSize = checkBytes;
		BIO_set_buffer_size(cl, workingBufferSize);
	}

	//aroth: put back all the stuff we just read, so that it can be read again, as normal
	BIO_set_buffer_read_data(cl, safeBuf, checkBytes);
	free(safeBuf);

	//reallocate buffers of the correct size
	free(buf);
	buf = malloc(workingBufferSize);
    request = malloc(workingBufferSize);
    response = malloc(workingBufferSize);
    url = malloc(workingBufferSize);
    loc_path = malloc(workingBufferSize);
    ...
    for(cl_11 = be_11 = 0;;) {
        ...
        strncpy(request, headers[0], workingBufferSize);				//aroth: this appears to load the 'GET ...' portion of the request into the first header, meaning that when parsing headers the first entry should be allocated the larger buffer size
        ...
        for(chunked = 0, cont = L_1, n = 1; n < MAXHEADERS && headers[n]; n++) {
            ...
            if(!regexec(&AUTHORIZATION, headers[n], 2, matches, 0)) {
                ...
                BIO_write(bb, "\n", 1);
                if((inlen = BIO_read(b64, buf, workingBufferSize - 1)) <= 0) {
        ...
        while(be == NULL && backend->be_type == 0) {
            ...
            if((sock = socket(sock_proto, SOCK_STREAM, 0)) < 0) {
                str_be(buf, workingBufferSize - 1, backend);
        ...
        if(cur_backend->be_type == 0) {
            for(n = 0; n < MAXHEADERS && headers[n]; n++) {
                ...
                if(lstn->rewr_dest && check_header(headers[n], buf) == HEADER_DESTINATION) {
                    ...
                    snprintf(buf, workingBufferSize, "Destination: http://%s%s", caddr, loc_path);
                    free(headers[n]);
                ...
                if(BIO_printf(be, "%s\r\n", headers[n]) <= 0) {
                    str_be(buf, workingBufferSize - 1, cur_backend);
                    end_req = cur_time();
        ...
        if(cur_backend->be_type == 0 && ssl != NULL) {
            ...
            if((cipher = SSL_get_current_cipher(ssl)) != NULL) {
                SSL_CIPHER_description(cipher, buf, workingBufferSize - 1);
                strip_eol(buf);
            ...
            if(lstn->clnt_check > 0 && x509 != NULL && (bb = BIO_new(BIO_s_mem())) != NULL) {
                ...
                get_line(bb, buf, workingBufferSize);
                ...
                while(get_line(bb, buf, workingBufferSize) == 0) {
        ...
        if(cl_11 && chunked) {
             ...
             * copy till EOF
             */
            while((res = BIO_read(cl_unbuf, buf, workingBufferSize)) > 0) {
        ...
        for(skip = 1; skip;) {
            ...
            strncpy(response, headers[0], workingBufferSize);
            be_11 = (response[7] == '1');
            ...
            for(chunked = 0, cont = -1L, n = 1; n < MAXHEADERS && headers[n]; n++) {
                ...
                case HEADER_LOCATION:
                    if(v_host[0] && need_rewrite(lstn->rewr_loc, buf, loc_path, v_host, lstn, cur_backend)) {
                        snprintf(buf, workingBufferSize, "Location: %s://%s/%s",
                ...
                case HEADER_CONTLOCATION:
                    if(v_host[0] && need_rewrite(lstn->rewr_loc, buf, loc_path, v_host, lstn, cur_backend)) {
                        snprintf(buf, workingBufferSize, "Content-location: %s://%s/%s",
                            (ssl == NULL? "http": "https"), v_host, loc_path);
                        free(headers[n]);
                        if((headers[n] = strdup(buf)) == NULL) {
            ...
            if(!no_cont) {
                ...
                if(be_11 && chunked) {
                    ...
                    if(is_readable(be, cur_backend->to)) {
                         ...
                         * copy till EOF
                         */
                        while((res = BIO_read(be_unbuf, buf, workingBufferSize)) > 0) {

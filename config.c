...
conf_init(const char *name)
{
    ...
    if((f_in[0] = fopen(name, "rt")) == NULL) {
        logmsg(LOG_ERR, "can't open open file %s", name);
        exit(1);
    }
    n_lin[0] = 0;
    cur_fin = 0;
...
{
	//aroth:  use of MAXBUF is okay here; we are parsing configuration data
    char        lin[MAXBUF];
    BACKEND     *res;
...
parse_HTTP(void)
{
    ...
    //aroth:  make the err414 message more useful
	int 		err414Len = 0;
    char		err414Msg[256];
    ...
    err414Len = sprintf(err414Msg, "Request URI is too long; Pound.MAXREQUEST=%du", MAXREQUEST);
    err414Msg[err414Len] = '\0';
    ...
    res->to = clnt_to;
    res->rewr_loc = 1;
    res->err414 = err414Msg;//"Request URI is too long";
...
config_parse(const int argc, char **const argv)
{
...
#endif
#ifdef  C_MAXREQUEST
            if(strcmp(C_MAXREQUEST, "0"))
                logmsg(LOG_DEBUG, "    --with-maxrequest=%s", C_MAXREQUEST);
#endif
#ifdef  C_OWNER
            if(strcmp(C_OWNER, ""))
    ...
    //aroth:  check to see if the file exists at the expected location; if it does not, fallback to an alternate location
	FILE* testFile = NULL;
	if ((testFile = fopen(conf_name, "rt")) == NULL) {
		logmsg(LOG_WARNING, "No configuration found at path=%s, will fall back to default location of /etc/pound/pound.cfg", conf_name);
